...
-- Created: 2026-02-04
-- Description: Adds tier column to profiles and creates purchases table
...
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS tier_purchased_at TIMESTAMPTZ;

-- 3. Add Stripe payment reference
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS stripe_payment_id TEXT;

-- 4. Create purchases table to track all purchases
...
-- SELECT id, email, tier, tier_purchased_at FROM profiles LIMIT 10;
-- SELECT COUNT(*) as total_profiles, tier FROM profiles GROUP BY tier;
